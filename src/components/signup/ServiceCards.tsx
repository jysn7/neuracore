    ...
    <div className="flex-2 flex py-14  bg-bg-dark w-full">
        <div className="flex text-white px-3 w-full flex-col items-center justify-center gap-6">
          <div className='flex flex-col justify-center items-center'>
            <h1 className="font-bold pt-8 md:pt-1 text-xl md:text-2xl text-center ">Powerful Innovation Tools</h1>
            <p className="mb-10 text-sm mt-16 md:mt-3 text-text-secondary w-full max-w-md text-center">
              Our comprehensive platform provides everything you need to transform ideas into
              impactful solutions
            </p>
          </div>
          <div className="flex flex-col md:flex-row gap-6 tracking-wide">
            {cards.map((card, index) => (
              ...
              >
                ...
                <p className="font-semibold my-2 text-brand-red text-xs">{card.subtitle}</p>
                <p className="text-xs mt-2 text-text-secondary whitespace-pre-line">{card.description}</p>
              </div>
            ))}
          </div>
        </div>
