...
import { 
  ...
  Menu, MoonIcon, Plus, Settings, Settings2Icon, SunIcon, TrophyIcon, 
    ...
    <nav className='h-[8vh] sticky top-0 right-0 left-0 z-30 border-b-2 border-bg-gray flex justify-between px-[6vw] items-center bg-bg'>
      ...
      <div className='flex p-2 gap-4'>
        ...
        <Link href="/submit-idea" className='hidden md:flex items-center justify-center'>
          <div className='text-bg-dark-gray bg-text-primary hover:bg-bg-gray hover:text-text-primary rounded-full p-2'>
            <Plus size={19} />
          </div>
        </Link>
        <Link href="/payment" className="hidden md:flex hover:text-white hover:bg-btn-primary-hover gap-1 text-xs font-semibold items-center text-brand-red border-2 px-2.5 py-0.5 rounded-lg">
          <MailIcon size={17}/> Subscribe
        </Link>
        <button onClick={toggleTheme} className="hidden md:flex items-center hover:bg-btn-secondary-hover justify-center text-text-primary bg-bg-dark-gray rounded-full p-2">
          {theme === 'dark' ? <SunIcon size={19}/> : <MoonIcon size={19}/>}
        </button>

        {/* Profile Dropdown */}
        <div className="hidden md:flex items-center relative" ref={profileRef}>
          <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="bg-bg-dark-gray rounded-full p-2">
            <UserRound size={21}/>
          </button>
          {isProfileOpen && (
            <div className="absolute right-0 top-9 mt-3 w-44 rounded-lg border border-bg-gray bg-bg-dark shadow-lg z-50">
              <Link href="/profile" className="flex items-center gap-2 px-4 py-2 text-sm hover:text-white hover:bg-bg-dark-gray">
                <UserRound size={21}/> My Profile
              </Link>
              <Link href="/profile#notifications" className="flex items-center gap-2 px-4 py-2 text-sm hover:text-white hover:bg-bg-dark-gray">
                <BellIcon size={16}/> Notifications
              </Link>
              <Link href="/settings" className="flex items-center gap-2 px-4 py-2 text-sm hover:text-white hover:bg-bg-dark-gray">
                <Settings2Icon size={16}/> Settings
              </Link>
              <Link href="/logout" className="flex items-center gap-2 px-4 py-2 text-sm text-red-400 hover:bg-red-500/10">
            ...
            <div className="absolute top-0 right-0 bottom-0 w-[70vw] sm:w-[60vw] bg-bg-dark shadow-xl flex flex-col justify-between">
              ...
              <div>
                ...
                <div className="flex flex-col mt-2">
                  ...
                  <button className='flex items-center gap-3 p-4 hover:bg-bg-dark-gray rounded-r-full'>
                    <MailIcon/> Subscribe
                  </button>
                </div>
              </div>
